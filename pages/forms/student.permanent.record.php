...
$tempFontSize = $fontsize;
$cellwidth = 76;
while ($pdf->GetStringWidth($row['strand_def']) > $cellwidth) {
    $pdf->SetFontSize($tempFontSize -= 0.1);
}
$pdf->Cell(77, 5, $row['strand_def'], 'B', 0);
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(2, 5, '', 0, 0);
...
$pdf->SetXY(10, 143.5);
$l = mysqli_query($conn, "SELECT * FROM tbl_students WHERE student_id = '$student_id'");
while ($rows = mysqli_fetch_array($l)) {

    $sum = mysqli_query($conn, "SELECT SUM(total_units) as UN FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_senior ON tbl_subjects_senior.subject_id = tbl_enrolled_subjects.subject_id where tbl_enrolled_subjects.student_id = '$student_id' AND tbl_enrolled_subjects.acad_year = '$acadyear' AND tbl_enrolled_subjects.semester = '$semester'");
    $row = mysqli_fetch_array($sum);
    $credited = mysqli_query($conn, "SELECT SUM(total_units) as UN FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_senior ON tbl_subjects_senior.subject_id = tbl_enrolled_subjects.subject_id where remarks = 'Passed' and tbl_enrolled_subjects.student_id = '$student_id' AND tbl_enrolled_subjects.acad_year = '$acadyear' AND tbl_enrolled_subjects.semester = '$semester'");
    $rowew = mysqli_fetch_array($credited);
    $sqls = mysqli_query($conn, "SELECT *,tbl_subjects_senior.subject_code,tbl_subjects_senior.subject_description FROM tbl_enrolled_subjects LEFT JOIN tbl_subjects_senior ON tbl_subjects_senior.subject_id = tbl_enrolled_subjects.subject_id where tbl_enrolled_subjects.acad_year = '$acadyear' AND tbl_enrolled_subjects.semester = '$semester' AND student_id = '$student_id'") or die($conn);
    $y = $pdf->Gety();
    $xy = 6.5;
    $i = 1;
    while ($roe = mysqli_fetch_array($sqls)) {
        ...
        $pdf->Cell(3, 5, '', 0, 1);
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(31, 6.5, $roe['subject_code'], '', 0);
        $pdf->Cell(2, 6.5, '', 0, 0);
        $fontsize = 10;
        $tempFontSize = $fontsize;
        $cellwidth = 94;
        while ($pdf->GetStringWidth($roe['subject_description']) > $cellwidth) {
            $pdf->SetFontSize($tempFontSize -= 0.1);
        }
        $pdf->Cell(95, 6.5, $roe['subject_description'], 'B', 0, 'C');
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(12, 6.5, $roe['total_units'], '', 0, 'C');
        $pdf->Cell(2, 6.5, '', 0, 0);
        $pdf->Cell(14, 6.5, $roe['numgrade'], '', 0, 'C');
        $pdf->Cell(2, 6.5, '', 0, 0);
        if ($roe['remarks'] == 'Passed') {
            $credits = $roe['total_units'];
        } else {
            $credits = 0;
        }
